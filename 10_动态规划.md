...
[1143. 最长公共子序列 - 力扣（LeetCode）](https://leetcode.cn/problems/longest-common-subsequence/description/)(LCS)
...
| **特性 (Feature)**                     | **392. 判断子序列**                                          | **115. 不同的子序列**                                        | **72. 编辑距离**                                             |
| -------------------------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
| **问题目标**                           | 判断 `s` 是否为 `t` 的子序列 (True/False)                    | `t` 在 `s` 中作为子序列出现的**总次数**                      | `word1` 转换成 `word2` 的**最少操作数**                      |
| **$dp[i][j]$ 定义**                    | `s[0..i-1]` 是否为 `t[0..j-1]` 的子序列                      | `s[0..i-1]` 中 `t[0..j-1]` 出现的**次数**                    | `word1[0..i-1]` 变为 `word2[0..j-1]` 的**最少操作数**        |
| **当字符相等时**  (`s[i-1] == t[j-1]`) | `dp[i][j] = dp[i-1][j-1]` (s 和 t 都匹配上了，同时前移)      | `dp[i][j] = dp[i-1][j-1] + dp[i-1][j]` (用 `s[i-1]` 匹配 + 不用 `s[i-1]` 匹配) | `dp[i][j] = dp[i-1][j-1]` (字符相等，不需要操作)             |
| **当字符不等时**  (`s[i-1] != t[j-1]`) | `dp[i][j] = dp[i][j-1]` (s 不动，在 t 的后续中继续找)        | `dp[i][j] = dp[i-1][j]` (必须不用 `s[i-1]` 匹配，即删掉 `s[i-1]`) | `dp[i][j] = 1 + min(...)`  `min(dp[i-1][j],`  (删除) `dp[i][j-1],` (插入) `dp[i-1][j-1])` (替换) |
| **初始化**                             | `dp[0][...] = true` (空 `s` 是任何 `t` 的子序列) `dp[...][0] = false` (非空 `s` 不是空 `t` 的子序列) | `dp[...][0] = 1` (空 `t` 在任何 `s` 中都出现 1 次) `dp[0][...] = 0` (非空 `t` 在空 `s` 中出现 0 次) | `dp[i][0] = i` (word1 删 i 次变空) `dp[0][j] = j` (空变 word2 需插 j 次) |

```cpp
class Solution {
public:
    int minDistance(string word1, string word2) {
        // 这道题非常的抽象，dp[i][j]是这两个word在这两个长度匹配时需要的操作步数
        int m = word1.size();
        int n = word2.size();
        vector<vector<int>> dp(m + 1, vector<int>(n + 1, 0));
        // 初始化 
...
public:
    int countSubstrings(string s) {
        ...
        int res = 0;
        // 为了保证我们在计算 dp[i][j] 时，dp[i+1][j-1] 已经计算好了，遍历顺序必须是
        for(int i = n - 1; i >= 0; i--){
            for(int j = i; j < n; j++){
                // 如果不是回文串
